...
export default function App() {
    ...
    <nav className="sticky top-0 z-50 bg-white border-b border-[#E5E7EB] px-6 h-16 flex items-center justify-between">
      <div className="flex items-center gap-3 cursor-pointer">
        ...
        <div className="flex items-baseline gap-1">
          <span className="font-extrabold text-xl tracking-tight text-[#0D0F1A]">SENTINEL</span>
          <span className="font-bold text-[#2D3BE0] tracking-wide">CORE</span>
        </div>
      </div>
    ...
    >
      <div className="text-center mb-10">
        ...
        <h1 className="text-5xl font-black text-[#0D0F1A] tracking-tight mb-4">
          Sentinel Core <span className="text-[#2D3BE0]">v2.4</span>
        </h1>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm border border-[#E5E7EB] p-8">
        ...
        <div className="grid grid-cols-2 gap-6 mb-6">
          ...
          <div className="col-span-1">
            ...
            <div className="animated-border-wrapper">
              <input type="number" name="loanAmount" value={formData.loanAmount} onChange={handleInputChange} placeholder="USD Millions"
                className="w-full px-4 py-3 text-sm focus:outline-none font-medium placeholder:font-normal" required />
            </div>
          </div>
        </div>
      ...
      <p className="text-center text-xs text-gray-400 mt-8 mb-4 max-w-sm mx-auto leading-relaxed">
        Institutional-grade encryption active. Data processed under Sentinel Core Governance Protocol v4.2.
      </p>
    </motion.div>
  );
  ...
  const renderResultsState = () => {
    ...
    // Strip markdown characters to make regex and parsing more reliable
    const strippedChairmanText = chairmanText.replace(/[*#]/g, '');

    // Parse decision
    let decisionStr = "APPROVED";
    const decisionMatch = strippedChairmanText.match(/FINAL DECISION:\s*(REJECT|CONDITIONAL|REFER|APPROVE|STRONG APPROVE)/i);
    if (decisionMatch) {
      const d = decisionMatch[1].toUpperCase();
      if (d.includes("REJECT")) decisionStr = "REJECTED";
      else if (d.includes("CONDITIONAL") || d.includes("REFER")) decisionStr = "CONDITIONAL";
      else decisionStr = "APPROVED";
    } else {
      if (strippedChairmanText.toUpperCase().includes("REJECT") && !strippedChairmanText.toUpperCase().slice(0, 500).includes("APPROVE")) {
        decisionStr = "REJECTED";
      } else if (strippedChairmanText.toUpperCase().includes("CONDITIONAL") || strippedChairmanText.toUpperCase().includes("REFER")) {
    ...
    const cleanChairman = strippedChairmanText ?
      strippedChairmanText.split('\n\n')
        ...
        .slice(0, 3).join('\n\n') :
      "The synthesis of this quarter's data reinforces our conviction in the Sentinel Core strategy. We are witnessing an unprecedented convergence of traditional asset stability and tech-driven growth vectors. This is not merely an investment; it is a calculated stewardship of multi-generational wealth.";

    // Score from logic
    const scoreMatch = strippedChairmanText.match(/SENTINEL SCORE:\s*(\d+)/i);
    const scoreValRaw = scoreMatch ? parseInt(scoreMatch[1]) * 8 : (decisionStr === 'APPROVED' ? 784 : (decisionStr === 'REJECTED' ? 412 : 620));
    const scoreData = [{ name: 'Score', value: scoreValRaw, fill: '#2D3BE0' }];

    // Dynamic Upside and Risk (Handle variations with/without apostrophe)
    const upsideMatch = strippedChairmanText.match(/Bull's strongest point:\s*(.+?)(?=\n|$)/i) || strippedChairmanText.match(/Bulls strongest point:\s*(.+?)(?=\n|$)/i);
    const strategicUpside = upsideMatch ? upsideMatch[1].trim() : "The current macro-economic alignment suggests a significant tailwind for the private equity ventures within the core cluster. We anticipate a projected appreciation of 14.8% over the trailing 18-month window, primarily driven by the systemic consolidation of the logistics sub-sector.";

    const riskMatch = strippedChairmanText.match(/Bear's strongest point:\s*(.+?)(?=\n|$)/i) || strippedChairmanText.match(/Bears strongest point:\s*(.+?)(?=\n|$)/i);
    const riskExposure = riskMatch ? riskMatch[1].trim() : "While the upside remains robust, the concentration in emerging market liquidity represents a non-trivial variance risk. Sentinel suggests a strategic hedge via short-dated Treasury puts to neutralize potential volatility spikes in the upcoming fiscal quarter.";
      ...
      >
        ...
        <div className="grid grid-cols-12 gap-8 mb-8">
          ...
          <TiltCard delay={0.1 * 0.15} className="col-span-8 bg-white rounded-2xl p-10 border border-[#E5E7EB] shadow-sm relative flex flex-col items-center justify-center min-h-[400px]">
            <div className={`absolute top-6 right-6 ${decisionStr === 'REJECTED' ? 'bg-[#E53E3E]/10 text-[#E53E3E]' : 'bg-[#00C48C]/10 text-[#00C48C]'} px-3 py-1 rounded-sm text-xs font-bold tracking-wider uppercase z-20`}>
              Q4 Assessment
            </div>

            <p className="text-xs uppercase tracking-[0.2em] font-semibold text-gray-400 mb-6 relative z-20">Sentinel Alpha Verdict</p>

            <motion.h2
              initial={prefersReducedMotion ? { scale: 1, rotate: 0, opacity: 1, color: decisionStr === 'REJECTED' ? '#E53E3E' : '#0D0F1A' } : { scale: 1.4, rotate: -3, opacity: 0, color: decisionStr === 'REJECTED' ? '#FF0000' : '#00C48C' }}
              animate={{ scale: 1, rotate: 0, opacity: 1, color: decisionStr === 'REJECTED' ? '#E53E3E' : '#0D0F1A' }}
            ...
            <motion.div
              ...
              className={`flex items-center gap-2 ${decisionStr === 'REJECTED' ? 'bg-[#E53E3E]/5' : 'bg-[#00C48C]/5'} px-4 py-2 rounded-full relative z-20`}
            >
              <div className={`${decisionStr === 'REJECTED' ? 'bg-[#E53E3E]' : 'bg-[#00C48C]'} rounded-full p-1`}>
                {decisionStr === 'REJECTED' ? <AlertTriangle className="w-4 h-4 text-white" /> : <CheckCircle2 className="w-4 h-4 text-white" />}
              </div>
              <span className={`text-sm font-bold ${decisionStr === 'REJECTED' ? 'text-[#E53E3E]' : 'text-[#00C48C]'} tracking-wide uppercase`}>
                {decisionStr === 'REJECTED' ? 'High Risk - Allocation Blocked' : 'Allocation Recommended'}
              </span>
            </motion.div>
          </TiltCard>
          ...
          <div className="col-span-4 flex flex-col gap-8">
            <TiltCard delay={1 * 0.15} className="bg-white rounded-2xl p-8 border border-[#E5E7EB] shadow-sm flex-1 flex flex-col items-center text-center justify-between">
              <h3 className="text-xs uppercase tracking-[0.1em] font-semibold text-gray-500 w-full text-center relative z-20">Sentinel Score</h3>
              ...
              <div className="w-48 h-48 relative my-2 z-20">
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    <PolarAngleAxis type="number" domain={[0, 800]} angleAxisId={0} tick={false} />
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <div className="flex items-baseline justify-center">
                    <span className="text-5xl font-black text-[#0D0F1A] tracking-tighter">{scoreVal}</span>
                    <span className="text-xs font-bold text-gray-400 ml-1">± 18</span>
                  </div>
                  <span className="text-[10px] font-bold text-gray-400 mt-1 uppercase tracking-widest">Of 800</span>
                </div>
              </div>

              <p className="text-sm text-[#6B7280] leading-relaxed relative z-20">
                Positioned in the <span className="font-bold text-[#2D3BE0]">top {decisionStr === 'REJECTED' ? '92%' : '4.2%'}</span>.
              </p>
            </TiltCard>
          </div>
        </div>
        ...
        >
          ...
          <div>
            ...
            <p className="italic text-[#6B7280] font-serif leading-relaxed text-[15px]">
              {strategicUpside}
            </p>
          </div>
          <div>
        ...
        >
          ...
          <p className="text-base text-[#0D0F1A] font-medium leading-[1.8] mb-10 max-w-4xl relative z-10 whitespace-pre-wrap">
            "{cleanChairman}"
          </p>

          <div className="flex items-center gap-4 relative z-10">
            <div className="w-12 h-12 rounded-full bg-gray-100 bg-[url('https://api.dicebear.com/7.x/avataaars/svg?seed=Elias&backgroundColor=b6e3f4')] bg-cover border border-[#E5E7EB]"></div>
            <div>
              <div className="text-[#0D0F1A] font-bold tracking-wide">Elias Thorne</div>
              <div className="text-gray-500 text-xs font-medium">Chief Investment Strategist</div>
            </div>
          </div>
        </motion.div>
  ...
  useEffect(() => {
    if (appState === 'results') {
      setTargetScore(Math.round(resultsData?.score || 742));
    }
        ...
        <footer className="border-t border-[#E5E7EB] bg-white py-6 mt-auto relative z-10">
          <div className="max-w-7xl mx-auto px-6 flex justify-between items-center text-xs">
            <div className="flex items-center gap-2 font-bold text-[#0D0F1A] tracking-wider">
              <Shield className="w-4 h-4 text-gray-400" /> SENTINEL CORE
            </div>
            ...
            <div className="text-gray-400 font-medium tracking-wide">
              © 2024 Sentinel Core Intelligence Systems. All Rights Reserved.
            </div>
          </div>
        </footer>
      </div>
