...
def synthesize_cam_text(all_outputs: dict) -> str:
    ...
    {str(all_outputs.get('parser', 'Not available'))[:2500]}
    
    RESEARCH INTELLIGENCE:
    {str(all_outputs.get('research', 'Not available'))[:2000]}
    
    FRAUD DETECTION:
    {str(all_outputs.get('fraud', 'Not available'))[:2000]}
    
    BULL AGENT BRIEF:
    {str(all_outputs.get('bull', 'Not available'))[:2000]}
    
    BEAR AGENT BRIEF:
    {str(all_outputs.get('bear', 'Not available'))[:2000]}
    
    CHAIRMAN'S VERDICT (FINAL DECISION):
    {str(all_outputs.get('chairman', 'Not available'))[:2500]}
    
    STRESS TEST RESULTS:
    {str(all_outputs.get('stress_test', 'Not available'))[:2500]}
    
    PRIMARY DUE DILIGENCE NOTES:
    {str(all_outputs.get('primary_notes', 'No site visit notes provided.'))[:2000]}
    """
    
    return call_llm("cam_generator", SYNTHESIS_PROMPT, user_message, max_completion_tokens=1500)
...
def create_cam_word_document(cam_text: str, loan_details: dict, 
    ...
    meta = [
        ...
        ('Loan Amount:', f"₹{loan_details.get('loan_amount', 'N/A')}"),
    ...
    disclaimer.add_run(
        "[WARN]  IMPORTANT: This memorandum is AI-generated by SENTINEL and constitutes "
    ...
    try:
        ...
        print(f"[WARN]  Warning: Could not create directory {output_dir}: {e}")
        output_dir = "output"
        os.makedirs(output_dir, exist_ok=True)
    ...
    for attempt in range(max_retries):
        try:
            ...
            print(f"[SUCCESS] CAM Document saved: {output_path}")
            save_successful = True
            break
        except PermissionError as e:
            if attempt < max_retries - 1:
                # File might be open in Word, try with a different name
                print(f"[WARN]  Attempt {attempt + 1}/{max_retries}: File locked, retrying with different name...")
                ...
                print(f"[FAIL] Permission denied: {output_path}")
                print(f"   Hint: Close any open CAM documents in Word and try again")
                raise
        except Exception as e:
            print(f"[FAIL] Error saving CAM document: {str(e)}")
            raise
...
def run_cam_generator(all_outputs: dict) -> tuple:
    ...
    """
    print("[DOC] Running CAM Generator Agent...")
    ...
    print("[SUCCESS] CAM Generation Complete")
