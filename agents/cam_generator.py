...
def synthesize_cam_text(all_outputs: dict) -> str:
    ...
    {all_outputs.get('parser', 'Not available')[:1500]}
    
    RESEARCH INTELLIGENCE:
    {all_outputs.get('research', 'Not available')[:1500]}
    
    FRAUD DETECTION:
    {all_outputs.get('fraud', 'Not available')[:1500]}
    ...
    {all_outputs.get('chairman', 'Not available')[:1500]}
    
    STRESS TEST RESULTS:
    {all_outputs.get('stress_test', 'Not available')[:1500]}
    
    PRIMARY DUE DILIGENCE NOTES:
    {all_outputs.get('primary_notes', 'No site visit notes provided.')[:1500]}
    """
