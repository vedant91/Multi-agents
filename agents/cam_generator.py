...
SYNTHESIS_PROMPT = """
You are QUANTISENSE's CAM Generation Agent. Synthesize all agent outputs into 
...
def synthesize_cam_text(all_outputs: dict) -> str:
    ...
    user_message = f"""Synthesize all agent outputs into a professional Credit Appraisal Memorandum for {loan.get('company_name', 'the company')}.
Write in formal banking language with specific data points.

DOCUMENT PARSER:
{all_outputs.get('parser', 'Not available')[:2000]}

RESEARCH:
{all_outputs.get('research', 'Not available')[:2000]}

FRAUD DETECTION:
{all_outputs.get('fraud', 'Not available')[:2000]}

BULL BRIEF:
{all_outputs.get('bull', 'Not available')[:1500]}

BEAR BRIEF:
{all_outputs.get('bear', 'Not available')[:1500]}

CHAIRMAN'S VERDICT:
{all_outputs.get('chairman', 'Not available')[:2000]}

STRESS TEST:
{all_outputs.get('stress_test', 'Not available')[:1500]}

CREDIT OFFICER NOTES:
{all_outputs.get('primary_notes', 'No notes.')[:1000]}
"""
...
def create_cam_word_document(cam_text: str, loan_details: dict, 
    ...
    meta = [
        ('Prepared by:', 'QUANTISENSE Credit Intelligence System v1.0'),
    ...
    disclaimer.add_run(
        "⚠️  IMPORTANT: This memorandum is AI-generated by QUANTISENSE and constitutes "
    ...
    run = footer_text.add_run(
        "QUANTISENSE v1.0 | Adversarial Credit Intelligence Platform\n"
