...
app = FastAPI(title="QUANTISENSE Core API")
...
def execute_quantisense(job_id: str, payload: dict, uploaded_file_paths: list, queue: asyncio.Queue):
    """Runs the blocking run_quantisense function and puts events onto the async queue."""
    ...
    try:
        ...
        # ── POST-PROCESS RESULTS (Structured Decision Parsing) ──
        # Perform the fragile parsing on the server so the frontend has consistent, clean state
        decision_str = "APPROVED"
        score_val = 72
        
        if "chairman" in results:
            chairman_raw = results.get('chairman', "")
            
            # 1. Strip all markdown for easier scanning
            stripped = re.sub(r'[*#_~`>]', '', chairman_raw).replace('[', '').replace(']', '')
            
            # 2. Extract decision (STRONG APPROVE, APPROVE, CONDITIONAL, REFER, REJECT)
            d_match = re.search(r'DECISION\s*:\s*(STRONG\s*APPROVE|APPROVE|CONDITIONAL|REFER(?:\s+TO\s+CREDIT\s+COMMITTEE)?|REJECT(?:ED)?)', stripped, re.IGNORECASE)
            if d_match:
                d = d_match.group(1).upper().strip()
                if d.startswith("REJECT"):
                    decision_str = "REJECTED"
                elif d.startswith("CONDITIONAL") or d.startswith("REFER"):
                    decision_str = "CONDITIONAL"
                else:
                    decision_str = "APPROVED"
            else:
                tail = stripped[-500:].upper()
                if re.search(r'\bREJECT(?:ED)?\b', tail) and not re.search(r'\bAPPROV', tail):
                    decision_str = "REJECTED"
                elif re.search(r'\bCONDITIONAL\b', tail) or re.search(r'\bREFER\b', tail):
                    decision_str = "CONDITIONAL"
                    
            # 3. Extract score
            s_match = re.search(r'QUANTISENSE SCORE:\s*(\d+)', stripped, re.IGNORECASE)
            if s_match:
                try:
                    score_val = int(s_match.group(1))
                except ValueError:
                    pass
        
        # Inject parsed, structured fields into the final payload
        results["parsed_decision"] = decision_str
        results["parsed_score"] = score_val
...
async def start_analysis(
    ...
    asyncio.create_task(asyncio.to_thread(execute_quantisense, job_id, payload, temp_file_paths, queue))
